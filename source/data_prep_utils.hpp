...
Grid<DataType> initializeGrid(const std::string& file_path,const std::vector<std::string>& variable_names)
{
	...
	try
	{
		...
		const auto& lon_shape=processor.getVariableShape("lon");
		const auto& lat_shape=processor.getVariableShape("lat");
		//const auto& water_u_shape=processor.getVariableShape("water_u");

		// Retrieve variable data
		const auto& lon_data=processor.getVariableData("lon").at(0.0);
		const auto& lat_data=processor.getVariableData("lat").at(0.0);
		const auto& water_u_data=processor.getVariableData("water_u").begin()->second;
		...
		if (lon_shape.size()==1 && lat_shape.size()==1)
		{
			...
			std::vector<DataType> lon_rad(n_lon);
			std::vector<DataType> lat_rad(n_lat);

			for (std::size_t i=0;i<n_lon;i++)
			{
				lon_rad[i]=lon_data[i]*grad_to_rad_factor;
			}

			for (std::size_t j=0;j<n_lat;j++)
			{
			lat_rad[j]=lat_data[j]*grad_to_rad_factor;
			}
			...
			for (std::size_t j=0;j<n_lat;j++)
			{
				for (std::size_t i=0;i<n_lon;i++)
				{
					...
					grid.lon_rad[idx]=lon_rad[i];
					grid.lat_rad[idx]=lat_rad[j];
				}
			}

			// Ensure that water_u_data size matches grid.num_points
			if (water_u_data.size()!=grid.num_points)
			{
				throw std::runtime_error("water_u data size does not match grid size.");
			}

			// Determine in_the_sea vector using water_u data
			for (std::size_t idx=0;idx<grid.num_points;idx++)
			{
				DataType u_value=water_u_data[idx];
				grid.in_the_sea[idx]=!std::isnan(u_value);
			}
		}
		else
		{
			throw std::runtime_error("Unsupported dimensions for 'lon' and 'lat' variables.");
		}
	}
	...
	TimeType time_step_interval)
{
	...
	std::map<TimeType,std::map<std::string,std::vector<DataType>>> data_at_time_step;
	...
	// (1) Debug-print actual time steps found
	std::cout<<"\n=== Actual time steps found in data files ===\n";
	std::cout<<"Earliest: "<<earliest_time<<"\n";
	std::cout<<"Latest:   "<<latest_time  <<"\n";
	std::cout<<"All actual time steps:\n";
	...
	// (2) Debug-print the expected time steps
	std::cout<<"=== Expected time steps ("<<num_time_steps<<") ===\n";
	...
	for (const auto& [time_step,file_path] : time_to_file_map)
	{
		try
		{
			...
			{
			// Time step is not in expected time steps (unlikely)
	...
	TimeType start_time,TimeType time_step)
{
	for (auto& [var_name,time_series_data] : variable_time_series)
	{
		...
		{
			auto& data=time_series_data[i];   // drop 'const'
...
// Trajectory: samples captured while backtracking
//struct Trajectory
//{
//	uint64_t seed_id{0};
//	int32_t  start_index{0};
//	std::vector<DataType> lon;   // float
//	std::vector<DataType> lat;   // float
//};
