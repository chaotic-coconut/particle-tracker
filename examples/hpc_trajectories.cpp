...
struct Trajectory {
  ...
  inline void append(float lon_r, float lat_r, float u_ms, float v_ms,
                     int32_t idx) {
    lon.push_back(lon_r);
    lat.push_back(lat_r);
    u.push_back(u_ms);
  ...
  inline void flush_to(pkd2::Writer &w_pos, pkd2::Writer &w_vel,
                       uint32_t stride = 1) {
    if (lon.empty())
      return;
    ...
    // Positions
    w_pos.add_traj_rad<float>(seed_id, lon.data(), lat.data(),
                              static_cast<uint32_t>(lon.size()), first_idx,
                              stride);
    // Velocities
    // lon channel = u, lat channel = v (units: m/s).
    w_vel.add_traj_rad<float>(seed_id, u.data(), v.data(),
                              static_cast<uint32_t>(u.size()), first_idx,
                              stride);
    lon.clear();
    lat.clear();
    u.clear();
...
static inline int64_t sec_to_idx_exact(int64_t t_s, int32_t dt_s) {
  if (dt_s <= 0)
    throw std::runtime_error("dt_s <= 0");
  if (t_s % dt_s == 0)
    return t_s / dt_s;
  return static_cast<int64_t>(
      std::llround(static_cast<double>(t_s) / static_cast<double>(dt_s)));
}

inline void normalize_seed_times_to_t2000(std::vector<SeedInit> &seeds,
                                          SeedTimeEpoch epoch) {
  if (epoch == SeedTimeEpoch::Since2000)
    return;
  for (auto &s : seeds) {
    s.t_start_s -= UNIX_TO_2000;
    if (s.has_stop)
      s.t_stop_s -= UNIX_TO_2000;
  }
}
...
inline void snap_seed_times_to_grid(std::vector<SeedInit> &seeds,
  ...
  for (auto &s : seeds) {
    ...
    if (s.has_stop)
      s.t_stop_s = snap_to_grid(s.t_stop_s, dt_s);
  }
}
...
// --------------------------------------------------------------------
static inline void enqueue_seeds(const std::vector<SeedInit> &seeds,
                                 int32_t dt_s) {
  for (const auto &s : seeds) {
    ...
    // Start point = lon_start_rad/lat_start_rad by our record-building logic in
    // main()
...
static inline Date month_start_from_t2000(int64_t t2000_s) {
  return dateFromSecondsSince2000(static_cast<TimeType>(t2000_s))
      .beginOfMonth();
}

static inline void bucketize_seeds(const std::vector<SeedInit> &seeds,
                                   SeedsByMonth &bym) {
...
static inline void activate_month(SeedsByMonth &bym, const Date &win_beg,
                                  const Date &win_end, int32_t dt_s) {
  ...
  const int64_t tmax =
      static_cast<int64_t>(secondsSince2000(win_end.addDays(1)));
...
void propagateWindow(const Date &win_beg, const Date &win_end,
  ...
  if (data_vars.size() < 2)
    throw std::runtime_error(
        "propagateWindow: need at least 2 data_vars (u,v)");
  ...
  // Window lower bound in *index* (exact: win_beg is midnight, base_dt_s
  // divides 86400 for 3h)
  ...
  const DataType search_radius =
      static_cast<DataType>(.08 * 1.1 / 180. * number_pi);
  ...
  std::atomic<uint64_t> stop_life{0}, stop_no_nb{0}, stop_fast{0}, stop_step{0},
      stop_seed0{0};
    ...
    oneapi::tbb::parallel_for(
        ...
        [&](const oneapi::tbb::blocked_range<std::size_t> &r) {
          ...
          for (std::size_t i = r.begin(); i != r.end(); i++) {
            ...
            while (p.reason == EndReason::none && p.time_idx >= t_beg_idx &&
                   (!p.has_stop || p.time_idx >= p.stop_idx)) {
              // If exactly at seed0 time, snap to seed0 position, record, and
              // finish.
              if (p.has_stop && p.time_idx == p.stop_idx) {
                p.lon = p.stop_lon;
                p.lat = p.stop_lat;

                if (p.time_idx < std::numeric_limits<int32_t>::min() ||
                    p.time_idx > std::numeric_limits<int32_t>::max()) {
              ...
              auto vals =
                  nb.interpolateVariables(t_s, neigh, md.spl, data_vars);
              DataType u = vals.at(data_vars[0]);
              DataType v = vals.at(data_vars[1]);

              p.traj.append(static_cast<float>(p.lon),
                            static_cast<float>(p.lat), static_cast<float>(u),
                            static_cast<float>(v),
                            static_cast<int32_t>(p.time_idx));

              const double spd =
                  std::sqrt(double(u) * double(u) + double(v) * double(v));
              ...
              {
                const double max_step_km = G.max_current_mps *
                                           double(base_dt_s) * G.step_slack /
                                           1000.;
                const double dr_km = std::sqrt(double(dx) * double(dx) +
                                               double(dy) * double(dy)) /
                                     1000.;
              ...
              while (p.lon <= -number_pi)
                p.lon += 2 * number_pi;
              while (p.lon > number_pi)
                p.lon -= 2 * number_pi;
              if (p.lat < -number_pi / 2)
                p.lat = -number_pi / 2;
              if (p.lat > number_pi / 2)
                p.lat = number_pi / 2;
  ...
  std::cerr << std::format(
      ...
      formatDate(win_beg), stop_life.load(), stop_no_nb.load(),
      stop_fast.load(), stop_step.load(), stop_seed0.load());
}
...
int main(int argc, char *argv[]) try {
  ...
  if (argc < 9) {
    std::cerr
        << "usage: " << argv[0] << "\n"
        << "  REL_START REL_END LEVEL TIMESTEP_H PATTERN1 PATTERN2 OUT_DIR "
           "SEEDS_PATH\n"
        << " [SEED_EPOCH] [VARLIST]\n"
  ...
  if (timestep_h <= 0) {
    std::cerr << "TIMESTEP_H must be positive.\n";
    return 1;
  }
  ...
  SeedTimeEpoch epoch = ends_with_ci(seeds_path, ".pkd2")
                            ? SeedTimeEpoch::Since2000
                            : SeedTimeEpoch::Unix;
  ...
  if (argi < argc && !is_flag(argv[argi])) {
    ...
    if (tok.find(',') != std::string::npos) {
      ...
      if (names.size() < 3) {
        std::cerr << "ERROR: variable list must contain lon_var,lat_var,at "
                     "least one data_var\n";
        return 1;
      }
  ...
  for (; argi < argc; ++argi) {
    ...
    if (a == "--keep-p") {
      if (argi + 1 >= argc) {
        std::cerr << "ERROR: --keep-p requires a value\n";
        return 1;
      }
      keep_p = std::stod(argv[++argi]);
    } else if (a == "--cap") {
      if (argi + 1 >= argc) {
        std::cerr << "ERROR: --cap requires a value\n";
        return 1;
      }
      ...
      if (argi + 1 >= argc) {
        std::cerr << "ERROR: --seed-point requires first|last\n";
        return 1;
      }
      const std::string v = lower(argv[++argi]);
      if (v == "first")
        seed_point = SeedPoint::First;
      else if (v == "last")
        seed_point = SeedPoint::Last;
      else {
        std::cerr << "ERROR: --seed-point must be first|last\n";
        return 1;
      }
    } else {
  ...
  if (ends_with_ci(seeds_path, ".pkd2") && epoch == SeedTimeEpoch::Unix) {
    std::cerr << "FATAL: SEEDS_PATH is .pkd2; seed times are already t2000. Do "
                 "NOT pass 'unix'.\n";
    return 2;
  }
  ...
  const auto pkd_pos_path =
      (out_dir / std::format("trajectories_{}h.pkd2", timestep_h)).string();
  const auto pkd_vel_path =
      (out_dir / std::format("velocities_{}h.pkd2", timestep_h)).string();

  pkd2::Writer pkd_pos(pkd_pos_path, T0_EPOCH_S, base_dt_s,
                       /*pos_scale=*/1'000'000u, /*zlib=*/6);
  pkd2::Writer pkd_vel(pkd_vel_path, T0_EPOCH_S, base_dt_s,
                       /*pos_scale=*/1'000'000u, /*zlib=*/6);
  ...
  std::vector<SeedInit> seeds;
  {
    ...
    if (ends_with_ci(seeds_path, ".pkd2")) {
      ...
      if (stops.size() != starts.size())
        throw std::runtime_error(
            "PKD2: starts.size() != stops.size() (corrupt TOC or mismatch)");

      seeds.reserve(starts.size());
      for (size_t i = 0; i < starts.size(); ++i) {
        if (starts[i].seed_id != stops[i].seed_id)
          throw std::runtime_error(
              "PKD2: seed_id mismatch between start/stop streams");
  ...
  std::cerr << "SEEDS month min="
            << formatDate(month_start_from_t2000(minmax.first->t_start_s))
            << " max="
            << formatDate(month_start_from_t2000(minmax.second->t_start_s))
            << "\n";
  ...
  for (const auto &s : seeds) {
    if (s.has_stop && s.t_stop_s > s.t_start_s) {
      throw std::runtime_error("Seed ordering invalid for backtracking: "
                               "t_stop_s > t_start_s for seed_id=" +
                               std::to_string(s.seed_id) +
                               ". Use --seed-point last for backtracking, or "
                               "fix seed construction.");
    }
  }
  ...
  while (cur_month >= first_month) {
    ...
    propagateWindow(win_beg, win_end, buf[older], buf[newer],
                    data_variable_names, dt_seconds, life_limit_seconds,
                    pkd_pos, pkd_vel, base_dt_s);

    for (auto it = all_particles.begin(); it != all_particles.end();) {
      auto &vec = it->second;
      vec.erase(std::remove_if(
                    vec.begin(), vec.end(),
                    [](const auto &p) { return p.reason != EndReason::none; }),
                vec.end());
      if (vec.empty())
