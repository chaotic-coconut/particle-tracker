 ...
 * @brief Time and spherical (lon/lat) interpolation utilities, including an experimental
 *        "neighbors-of-neighbors" spatial derivative estimator.
 *
 * ## What this header provides
 * - **Time interpolation** of scalar fields using a cubic B-spline (Boost).
 * - **Spatial neighbor queries** on a sphere (lon/lat) via a KD-tree (nanoflann)
 *   using a fast approximate metric suitable for KD-tree pruning.
 * - **Spatial interpolation** using Gaussian weights derived from *true* great-circle distance.
 * - **Experimental** spatial derivative estimation using a two-level neighborhood
 ...
 *   - or `<boost/math/interpolators/cubic_b_spline.hpp>` (older / deprecated in some newer Boost)
 * - nanoflann (header-only) for KD-tree support.
 *
 * ## Error policy
 * This header prefers **loud failures** over silent bad behavior:
 * - If enable nanoflann support and the header cannot be found: **compile-time error**.
 * - If disable nanoflann support but try to instantiate KD-tree dependent types: **compile-time error**.
 *
 * ## Threading
 ...
 *
 * If set to 0, KD-tree dependent classes/types in this header become unavailable
 * and will produce a compile-time error if instantiated.
 */
...
#else
  #error "particle_tracker: Boost spline header not found. Need either <boost/math/interpolators/cardinal_cubic_b_spline.hpp> or <boost/math/interpolators/cubic_b_spline.hpp>."
#endif
...
/**
 * @class TimeSpline
 * @brief Wrapper around a Boost cubic spline interpolator for uniformly spaced samples.
 *
 ...
 * - Evaluate with `spline(t)` and time-derivative with `spline.prime(t)` if supported.
 *
 * ### Notes
 * - This class stores the data and the spline object.
 * - The underlying Boost spline type depends on what Boost provides:
 *   - `boost::math::interpolators::cardinal_cubic_b_spline` (preferred when available)
...
public:
    ...
    {
        if (values.empty())
            throw std::runtime_error("particle_tracker::TimeSpline: no data to create spline (values is empty).");

#if PARTICLE_TRACKER_HAS_BOOST_CUBIC_B_SPLINE
        // Old Boost API accepts pointer + size + (t0, dt)
        spline = boost::math::cubic_b_spline<DataType>(values.data(), values.size(), start_time, time_step);
#elif PARTICLE_TRACKER_HAS_BOOST_CARDINAL_CUBIC_B_SPLINE
        // New Boost API uses interpolators::cardinal_cubic_b_spline(values, t0, dt)
        spline = boost::math::interpolators::cardinal_cubic_b_spline<DataType>(values, start_time, time_step);
#endif
    }
};
...
/**
 * @struct PointCloud
 * @brief Minimal dataset adaptor for nanoflann storing (lon, lat) in a flat array.
 *
 ...
 *   ```
 * - After KD-tree returns candidate neighbors, **true** great-circle distance is
 *   computed via `GCD()` / `GCD_deriv()` and used to compute weights.
 *
 * ### Dateline behavior
 * - Longitude differences are wrapped into (-pi, pi] for continuity across the dateline.
 */
...
struct LonLatDistanceAdaptor
{
     ...
     */
    inline DistanceType evalMetric(const DataType* a, const DataType* b, std::size_t /*size*/) const
    {
        DataType dlon = a[0] - b[0];
        if (dlon < -static_cast<DataType>(pt_pi)) dlon += static_cast<DataType>(pt_two_pi);
        else if (dlon > static_cast<DataType>(pt_pi)) dlon -= static_cast<DataType>(pt_two_pi);
    ...
    /**
     * @brief Squared approximate distance between query point a and dataset point idx_b.
     */
    inline DistanceType evalMetric(const DataType* a, std::size_t idx_b, std::size_t size) const
    {
    ...
    /**
     * @brief Incremental squared contribution along a single dimension (KD-tree pruning).
     *
    ...
    inline DistanceType accum_dist(const T a, const T b, IndexType dim) const
    {
        ...
        if (dim == 0)
        {
            if (diff < -static_cast<DistanceType>(pt_pi)) diff += static_cast<DistanceType>(pt_two_pi);
            else if (diff > static_cast<DistanceType>(pt_pi)) diff -= static_cast<DistanceType>(pt_two_pi);
        }
...
using KDTree = nf::KDTreeSingleIndexAdaptor<
    LonLatDistanceAdaptor<DataType, PointCloud<DataType>>,
    PointCloud<DataType>,
    2>;
...
template<typename...>
struct pt_dependent_false : std::false_type {};

/**
 * @brief Dummy KDTree type when nanoflann is disabled.
 *
 * Any attempt to instantiate NeighborsData / NeighborWithNeighborsData with this
...
struct KDTree
{
    static_assert(pt_dependent_false<DataType>::value,
                  "particle_tracker: KDTree is unavailable because PARTICLE_TRACKER_HAS_NANOFLANN=0. "
 ...
 *
 * Fields:
 * - @ref ind: index into grid coordinate arrays (xx_sparse/yy_sparse) and per-node data arrays.
 * - @ref dist: great-circle distance (from @c GCD), **not** KD-tree metric distance.
 * - @ref weight: Gaussian weight, typically `exp(-(dist*dist)/shape)`.
 *
 * @note This is a lightweight, POD-like aggregate in practice (trivially copyable
 *       if DataType is), but it has a user-provided constructor, so it is **not**
 *       a "POD type" in the strict C++03 sense. In modern C++, the relevant traits are:
...
struct Neighbor
{
    ...
    Neighbor(std::size_t index = 0, DataType distance = DataType{}, DataType node_weight = DataType{})
...
/**
 * @class NeighborsData
 * @brief Spatial neighbor query + Gaussian weighting + multi-variable interpolation.
 *
 * @tparam DataType numeric type.
 ...
 * - `shape`  : Gaussian length scale for weights in terms of **great-circle distance**
 *
 * ### Dateline policy
...
public:
    NeighborsData(const KDTree<DataType>& idx,
                  const std::vector<DataType>& xxs,
                  const std::vector<DataType>& yys,
                  DataType r2,
    ...
    /**
     * @brief Recompute dist/weight for an existing neighbor list without changing indices.
     *
     ...
     * @param neighbors neighbor list produced by computeNeighbors/updateNeighborWeights
        ...
        const std::vector<std::string>& variable_names)
    {
        ...
        for (const auto& nbr : neighbors)
        {
            for (const auto& var_name : variable_names)
            {
                ...
                {
                    throw std::out_of_range("particle_tracker::NeighborsData: index " + std::to_string(nbr.ind) +
...
class NeighborsData
{
    static_assert(pt_dependent_false<DataType, TimeType>::value,
                  "particle_tracker::NeighborsData is unavailable because PARTICLE_TRACKER_HAS_NANOFLANN=0. "
                  "Enable nanoflann support or avoid KD-tree dependent APIs.");
};
...
//------------------------------------------------------------------------------
// Experimental derivative estimator: NeighborPrime, NeighborWithNeighbors, VariableDerivatives
//------------------------------------------------------------------------------
 ...
 * - @ref dist_prime: array [d, ddx, ddy] returned by @c GCD_deriv(primary, secondary).
 *   - dist_prime[0] = d    : great-circle distance
 *   - dist_prime[1] = ddx  : partial derivative of distance w.r.t. x-like coordinate
 *   - dist_prime[2] = ddy  : partial derivative of distance w.r.t. y-like coordinate
 * - @ref weight_prime: Gaussian weight `exp(-(d*d)/shape)` based on dist_prime[0].
 *
 */
template<typename DataType>
struct NeighborPrime
{
    ...
    NeighborPrime(std::size_t index_prime = 0,
                  std::array<DataType, 3> distance_prime = {DataType{}, DataType{}, DataType{}},
 ...
 */
template<typename DataType>
struct NeighborWithNeighbors : Neighbor<DataType>
{
    std::vector<NeighborPrime<DataType>> neighbors_prime;

    NeighborWithNeighbors() = default;

    NeighborWithNeighbors(std::size_t index,
                          DataType distance,
 ...
 * @brief Output container: value + time derivative + (heuristic) spatial derivatives.
 *
 * @tparam DataType numeric type.
 */
template<typename DataType>
 ...
 * @brief Two-level neighborhood builder + heuristic spatial derivative estimator.
 *
 * @tparam DataType numeric type.
 * @tparam TimeType time type.
 *
 ...
 * - If secondary neighborhood search returns empty for a primary neighbor, the primary neighbor
 *   is **skipped** (because derivatives cannot be formed). This changes the primary set.
 *   This is intentional and explicit; it prevents silent garbage derivatives.
 */
template<typename DataType, typename TimeType = double>
class NeighborWithNeighborsData
{
...
public:
    ...
    void computeNeighbors(std::vector<NeighborWithNeighbors<DataType>>& neighbors, DataType x, DataType y)
    {
        ...
        for (const auto& result : indices_dists)
        {
            ...
            // True great-circle distance query -> primary neighbor (for primary weights).
            ...
            {
                // Loud behavior: do not keep a primary neighbor with no secondary neighborhood
                // because derivative estimation would be undefined/noisy.
                continue;
            }
            ...
            for (const auto& result_prime : indices_dists_prime)
            {
                ...
                {
                    throw std::out_of_range("particle_tracker::NeighborWithNeighborsData: index " +
                ...
                nbr_prime.dist_prime = GCD_deriv(query_point_prime[0], query_point_prime[1], lon_j, lat_j);
    ...
    /**
     * @brief Update only primary (query->primary) distances and weights for an existing neighbor list.
     *
    ...
    /**
     * @brief Interpolate value/time-derivative and estimate spatial derivatives for requested variables.
     *
     * @param t evaluation time
        ...
        DataType shape) const
    {
        ...
        for (const auto& nbr : neighbors)
        {
            ...
            for (const auto& name : variable_names)
            {
                ...
                if (nbr.ind >= splines.size())
                {
                    throw std::out_of_range("particle_tracker::NeighborWithNeighborsData: index " + std::to_string(nbr.ind) +
                                            " out of bounds for variable '" + name + "'.");
                }
            ...
            for (const auto& nbr_prime : nbr.neighbors_prime)
            {
                ...
                // IMPORTANT: den_prime must be independent of number of variables requested.
                ...
                for (const auto& name : variable_names)
                {
                    ...
                    {
                        throw std::out_of_range("particle_tracker::NeighborWithNeighborsData: index " + std::to_string(nbr_prime.ind_prime) +
        ...
        if (den > static_cast<DataType>(1e-16))
        {
            for (const auto& name : variable_names)
            {
                ...
                vd.spatial_derivative_x = (static_cast<DataType>(2) / shape) * (dx_acc[name] / den);
                vd.spatial_derivative_y = (static_cast<DataType>(2) / shape) * (dy_acc[name] / den);
...
{
    static_assert(pt_dependent_false<DataType, TimeType>::value,
                  "particle_tracker::NeighborWithNeighborsData is unavailable because PARTICLE_TRACKER_HAS_NANOFLANN=0. "
                  "Enable nanoflann support or avoid KD-tree dependent APIs.");
};
