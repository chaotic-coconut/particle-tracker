...
#pragma pack(pop)
    ...
    void emit_block(uint64_t seed_id,
            const double* lon_r,
            const double* lat_r,
            uint32_t n,
            int32_t start_index,
            uint16_t stride)
    {
        ...
        if (!dlon_.empty())
        {
        std::memcpy(raw_.data() + sizeof(bh),
                dlon_.data(),
                dlon_.size() * sizeof(int32_t));
        ...
        if (bound > static_cast<size_t>(std::numeric_limits<uint32_t>::max()))
            throw std::runtime_error("pkd2: zstd compressBound exceeds 4GiB (format limit)");

        comp_.resize(bound);

        const size_t z = ZSTD_compress(comp_.data(), bound,
                           raw_.data(), raw_bytes,
                           lvl_);
        if (ZSTD_isError(z))
            throw std::runtime_error(std::string("ZSTD_compress: ") + ZSTD_getErrorName(z));

        if (z > static_cast<size_t>(std::numeric_limits<uint32_t>::max()))
            throw std::runtime_error("pkd2: compressed block exceeds 4GiB (format limit)");
        ...
        if (static_cast<size_t>(bound) > static_cast<size_t>(std::numeric_limits<uint32_t>::max()))
            throw std::runtime_error("pkd2: zlib compressBound exceeds 4GiB (format limit)");
        ...
        if (static_cast<size_t>(z) > static_cast<size_t>(std::numeric_limits<uint32_t>::max()))
            throw std::runtime_error("pkd2: compressed block exceeds 4GiB (format limit)");
        ...
        if (off < 0) throw std::runtime_error("pkd2: ftell failed");

        const size_t nw = std::fwrite(comp_.data(), 1, comp_sz, f_);
        if (nw != comp_sz) throw std::runtime_error("pkd2: fwrite failed");
